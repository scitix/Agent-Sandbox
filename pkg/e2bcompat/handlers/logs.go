...
//
// The native service owns the choice of source: a live sandbox's lines come
// from its Pod, and once the Pod is recycled they come from the central log
// service the lines were shipped to. This side only projects the result into
// the E2B shape, so both API surfaces answer a finished sandbox identically.
func (s *Server) fetchLogEntries(ctx context.Context, sandboxID string, q logQuery) ([]e2bgen.SandboxLogEntry, *apidomain.AppError) {
	...
	for i := range result.Entries {
		entry := e2bgen.SandboxLogEntry{
			Message: result.Entries[i].Log,
			Level:   parseLogLevel(result.Entries[i].Log),
			Fields: map[string]string{
				"container": result.Entries[i].Container,
				"source":    source,
			},
		}
