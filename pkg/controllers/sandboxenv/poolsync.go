...
func (r *SandboxEnvReconciler) updateMemberPoolIfDrifted(
	...
	// Gateway and MaxUnavailable are SandboxPoolSpec siblings (not part of
	// EmbeddedSandboxTemplate), so they need explicit drift checks. Gateway
	...
	gatewayDrift := !equality.Semantic.DeepEqual(pool.Spec.Gateway, want.Spec.Gateway)
	...
	if !labelDrift && !annotationDrift && !policyDrift && !startupDrift && !idleDrift &&
		!replicasDrift && !embeddedDrift && !gatewayDrift && !maxUnavailableDrift {
		return nil
	}
	...
	return retry.RetryOnConflict(retry.DefaultRetry, func() error {
		...
		current.Spec.Gateway = want.Spec.Gateway
