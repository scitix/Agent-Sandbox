...
This repository is published publicly. Internal-only identifiers must never
appear in it: real cluster codenames, internal hostnames, private subnets, or
datacentre names. Use neutral placeholders instead.

| Instead of | Use |
|-----------|-----|
| a real cluster codename | `foo`, `bar`, `baz` |
| a real Prometheus cluster label | `prod-foo`, `prod-bar` |
| an internal hostname | `gw.example.com`, `gw-a.example.internal` |
| a real private IP | `10.0.0.1`, `192.0.2.1` |

`gitleaks` enforces this. `.gitleaks.toml` carries the credential ruleset and
the false-positive allowlists. It deliberately does not enumerate the internal
identifiers themselves -- writing that list into a public file would publish the
inventory the rules exist to keep out -- so the deny-list is layered on from the
private release tooling when the repo is checked out inside it. The hook prints
which layer it used.

```bash
make install-hooks   # pre-commit gate: scans staged changes
make scan-secrets    # scan the full history on demand
```

Two layers run it here: the pre-commit hook, which scans staged changes, and the
`Secret Scan` GitHub Actions workflow, which scans the PR range on pull requests
and the full history on pushes to `develop`/`main`. Release tooling scans the
whole history again before anything is published.

Bypass the hook for one commit with `git commit --no-verify`, or skip just this
check with `HOOK_SKIP=secrets git commit ...`. CI has no bypass.
...
| Pool autoscaler | [`pkg/controllers/sandboxpool/autoscalingstate/doc.go`](pkg/controllers/sandboxpool/autoscalingstate/doc.go) | Load → Decide → Commit. Two things are easy to get wrong: every member lookup must be scoped to the local cluster segment (pool names repeat across segments, and a write to a foreign one is never read back — the Pool silently never grows), and events must be throttled (the decision logic is level-triggered, so a Pool that cannot grow re-posts the same event every cooldown forever). |
| Plugin system | [`pkg/framework/`](pkg/framework/) | Plugin (lifecycle hooks) + Provider (data source abstraction) interfaces |
| Pool sizing rule | [`pkg/framework/providers/quota/template_policy.go`](pkg/framework/providers/quota/template_policy.go) | `PoolSizing` says how an Env's member pools must be sized: `billed` (a quota AND an instance type), `free-form` (inlineResources only, instanceType and the quota label refused), or `either` (this deployment states no rule — the open-source behaviour, and the one that keeps an InstanceType catalog usable with no quota backend). The rule itself lives on the **optional** `quota.TemplatePolicy` interface, implemented only by a deployment that has one; the API server stamps the answer onto every Env (`agentbox.navix.sh/pool-sizing`) at create and the Env reconciler re-stamps it on each pass, so a template that gains or loses billing re-stamps its Envs and existing Envs are backfilled. The write path does not trust the label — `AddMember` re-derives it from the template before validating the body. |
