...
import { Box, RefreshCw, TrendingUp, Users, UserCheck } from "lucide-react"
...
export default function OverviewPage() {
  ...
  // Cluster scope selector, in the row below the page header. Neither the route
  // nor the session pins it: an API key authenticates against every cluster, so
  // the default is every cluster and only this picker narrows it.
  ...
  return (
    <div className="flex h-full w-full flex-col overflow-hidden">
      {/* Controls only — no second title. The page title, the cluster switcher
          and the live badge are the shared <PageHeader>'s, which is what puts
          this page's first line where every other page's is. */}
      <div className="border-border flex shrink-0 flex-wrap items-center gap-3 border-b px-6 py-3">
        <p className="text-muted-foreground max-w-2xl text-xs leading-relaxed">
          {t("overview.scopeHint")}
        </p>
