...
/**
 * GET /api/prometheus/http-request-duration-native
 *
 * Returns HTTP request latency percentiles for the Native API over a time window.
 * * Admin only.
 *
 * Query params:
 *   cluster=<required>
...
export const GET = withPrometheusRoute(
  ...
  async ({ config, sel, timeRange }) => {
    ...
    const queries = PERCENTILES.map(
      ({ q }) =>
        `histogram_quantile(${q}, sum by (le) (rate(agentbox_http_request_duration_seconds_bucket{${sel},api="native"}[${rateWindow}])))`,
    )
