...
/**
 * GET /api/prometheus/schedule-success-rate
 *
 * Per-second rate of sandbox schedule attempts (Idle → Starting/Running
 * inplace update with target=running), broken down by result label.
...
export const GET = withPrometheusRoute(
  ...
  async ({ config, sel, timeRange }) => {
    ...
    const queries = [
      {
        name: "Success",
        query: `sum(rate(agentbox_inplace_update_total{${sel},target="running",result="success"}[${rateWindow}]))`,
      },
      {
        name: "Conflict",
        query: `sum(rate(agentbox_inplace_update_total{${sel},target="running",result="conflict"}[${rateWindow}]))`,
      },
      {
        name: "Error",
        query: `sum(rate(agentbox_inplace_update_total{${sel},target="running",result="error"}[${rateWindow}]))`,
      },
    ]
