 ...
 *
 * Returns ClaimIdlePod latency percentiles (P99/P95/P90/P50) over a time window.
...
export const GET = withPrometheusRoute(
  ...
  async ({ config, sel, timeRange }) => {
    ...
    const queries = PERCENTILES.map(
      ({ q }) =>
        `histogram_quantile(${q}, sum by (le) (rate(agentbox_sandbox_claim_duration_seconds_bucket{${sel},outcome="success"}[${rateWindow}])))`,
    )
