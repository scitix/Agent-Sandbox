...
/**
 * GET /api/prometheus/envoy-upstream-rate
 *
 * Returns Envoy external upstream request rate trends over a time window,
 * split by HTTP response code class. Admin only.
 *
 * Query params:
 *   cluster=<required>
...
export const GET = withPrometheusRoute(
  ...
  async ({ config, filters, timeRange }) => {
    ...
    const queries = [
      {
        name: "2xx",
        query: `sum(rate(envoy_cluster_external_upstream_rq_xx{${envoySel},envoy_response_code_class="2"}[${rateWindow}]))`,
      },
      {
        name: "4xx",
        query: `sum(rate(envoy_cluster_external_upstream_rq_xx{${envoySel},envoy_response_code_class="4"}[${rateWindow}]))`,
      },
      {
        name: "5xx",
        query: `sum(rate(envoy_cluster_external_upstream_rq_xx{${envoySel},envoy_response_code_class="5"}[${rateWindow}]))`,
      },
    ]
