 ...
 *
 * Returns CPU usage rate for a sandbox over a time range.
 ...
 *
 * Response: TimeSeriesData with series named "CPU"
 *   Unit: cores (fractional, e.g. 0.5 = half a core)
 *
...
export const GET = withPrometheusRoute(
  ...
  async ({ config, request }) => {
    ...
    // For short durations (≤ 5 min) use irate for per-scrape-interval precision.
    // For longer durations use rate with a window = 2× step to smooth noise.
  ...
  * on(pod) group_left(sandbox_id)
  agentbox_sandbox_running_info{${sandboxSelector}}
    ...
    const series = result ? rangeResultToSeries(result, ["CPU"]) : []
