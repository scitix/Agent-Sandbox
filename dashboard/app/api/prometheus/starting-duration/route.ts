...
/**
 * GET /api/prometheus/starting-duration
 *
 * Returns Sandbox Starting (claimedAt → startedAt) latency percentiles
 * (P99/P95/P90/P50) over a time window. Only outcome=success is charted;
 * canceled sandboxes never reached Running and would skew the percentiles.
 *
...
export const GET = withPrometheusRoute(
  ...
  async ({ config, sel, timeRange }) => {
    ...
    const queries = PERCENTILES.map(
      ({ q }) =>
        `histogram_quantile(${q}, sum by (le) (rate(agentbox_sandbox_starting_duration_seconds_bucket{${sel},outcome="success"}[${rateWindow}])))`,
    )
