 ...
 *
 * Returns Sandbox running duration percentiles (P99/P95/P90/P50) over a time window.
 * Measures total sandbox lifetime from claimedAt to terminatedAt.
...
export const GET = withPrometheusRoute(
  ...
  async ({ config, sel, timeRange }) => {
    ...
    const queries = PERCENTILES.map(
      ({ q }) =>
        `histogram_quantile(${q}, sum by (le) (rate(agentbox_sandbox_running_duration_seconds_bucket{${sel}}[${rateWindow}])))`,
    )
