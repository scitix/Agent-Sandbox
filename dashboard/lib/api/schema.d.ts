...
export interface paths {
    ...
    "/volumes": {
         ...
         * @description Returns the Bound PersistentVolumeClaims in the caller's namespace, which are exactly the claims a SandboxEnv may mount through `overrides.volumes`. There is deliberately no namespace parameter: the namespace is derived from the caller's identity, and that derivation is the authorisation boundary. Empty when the feature is disabled (see `volumes` on `/feature-gates`).
         */
...
export interface components {
    schemas: {
        ...
        CreateSandboxRequest: {
            /**
             * @description Where to allocate the sandbox from:
             *
        ...
        /**
         * @description How the member Pools of a template — and therefore of an Env bound to
         ...
         * @enum {string}
         */
        PoolSizing: "billed" | "free-form" | "either";
        /**
         * @example {
         *       "name": "poolname",
         *       "namespace": "t-team-user",
        ...
        SandboxPool: {
             ...
             */
            createdAt?: string;
            /** @description Name of the SandboxEnv that owns this pool (resolved from OwnerReferences). Every pool is derived from an Env; empty only for a pool with no owning Env. */
            owningEnv?: string;
        };
        SandboxPoolEnvelope: {
        ...
        UpsertSandboxPoolRequest: {
             ...
             */
            multiplier?: number;
            /** @description Explicit per-Pool resource requests/limits. Required — and used alone — when the Pool's Env is free-form (env.poolSizing=free-form), where it is the whole of the Pod's sizing; on a billed Env it is optional and instead combined with instanceType as the rounded-down actual Pod request (must fit within instanceType × multiplier). Sets both requests and limits. */
        ...
        /**
         * @description Desired state of this member's editable bounds. This is a PUT and it
        ...
        EnvOverrides: {
            ...
            updateStrategy?: components["schemas"]["EnvUpdateStrategy"];
            /**
             * @description Mount existing PersistentVolumeClaims from this Env's namespace into the
             ...
             */
            volumes?: components["schemas"]["EnvVolumeMount"][];
        };
        /** @description One PersistentVolumeClaim, or one subtree of it, mounted into the sandbox container. */
        EnvVolumeMount: {
            /** @description Name of an existing, Bound PersistentVolumeClaim in this Env's namespace. Cross-namespace mounts are not possible in Kubernetes, which is the authorisation boundary: a caller only ever reaches claims in the namespace their identity resolved to. */
            claimName: string;
            /** @description Absolute path inside the sandbox container. Must not be '/' or '/mnt', must not sit inside a reserved path (/proc, /sys, /dev, /etc, /var/run, /var/lib/kubelet), and must not collide with or nest against a path the Template already mounts. */
            mountPath: string;
            /** @description Mount a subtree of the volume instead of its root. Relative, no '..' segments. Recommended when the backing PersistentVolume is exposed at its filesystem root. Note kubelet creates a missing subPath directory, so a mistyped value mounts an empty directory rather than failing. */
            subPath?: string;
            /**
             * @description Defaults to true. Enforced on the Kubernetes volume source rather than the mount, so no container can request read-write for the same claim. Set false only when the agent must write: sandbox code runs as root with passwordless sudo and can delete anything writable. Read-only is a container-runtime bind flag — a Template whose pod spec can reach the host mount namespace (privileged, SYS_ADMIN, Bidirectional propagation, hostPath) cannot enforce it, and such a combination is rejected unless an administrator has opted the Template out.
             * @default true
             */
            readOnly: boolean;
        };
        ...
        SandboxEnv: {
            ...
            createdAt?: string;
            /**
             * @description Rendered Markdown documentation for this Env, sourced from the linked SandboxTemplate's
            ...
            envDocs?: string;
            /**
             * @description How this Env's member Pools must be sized, derived from the bound template's own poolSizing and carried on the Env (the agentbox.navix.sh/pool-sizing label) by the API server on create and by the Env reconciler on every pass. A template that gains or loses its billing annotations therefore re-stamps the Envs that reference it within a reconcile.
             ...
             */
            poolSizing?: components["schemas"]["PoolSizing"];
        };
        SandboxEnvEnvelope: {
        ...
        FeatureGates: {
            ...
            /**
             * @description True when mounting existing PersistentVolumeClaims into sandboxes is enabled (the volumes panel in the Env upsert sheet, `/volumes` listing endpoint). When false the server also rejects a non-empty `overrides.volumes`, so this is a kill switch and not only a UI hint.
             * @example false
             */
            volumes?: boolean;
        };
        ...
        ListVolumesResult: {
            ...
            /** @description Number of mountable claims returned. */
            total: number;
        };
        /** @description One PersistentVolumeClaim in the caller's namespace that a SandboxEnv may mount. */
        VolumeItem: {
            /** @description The claim's name — the value to put in `overrides.volumes[].claimName`. */
            claimName: string;
            /** @description Human-readable name, resolved from the first non-empty label listed in the deployment's `--volume-display-name-labels`. Falls back to claimName when no label matches or none is configured, so this is always safe to render directly. */
            displayName?: string;
            /** @description Requested capacity, e.g. '5Ti'. Absent when the claim reports none. */
            capacity?: string;
            /** @description The claim's access modes, e.g. ['ReadWriteMany']. */
            accessModes?: string[];
            /** @description The claim's storageClassName, when set. */
            storageClass?: string;
            /** @description Claim phase. Only Bound claims are returned, so this is always 'Bound' today; present so a future filter relaxation stays legible. */
            phase: string;
            /** @description The claim's labels, verbatim. Returned so a caller can apply its own presentation rules without this API having to know the storage platform's label vocabulary. */
            labels?: {
                [key: string]: string;
            };
        };
        ...
        responses: {
            ...
            200: {
                ...
                content: {
                     ...
                     *       "items": [
                     *         {
                     *           "claimName": "team-user-41",
                     *           "displayName": "zystore",
                     *           "capacity": "5Ti",
                     *           "accessModes": [
                     *             "ReadWriteMany"
                     *           ],
                     *           "storageClass": "example-shared-fs",
                     *           "phase": "Bound"
                     *         },
                     *         {
            ...
            /** @description Internal error — the claims could not be read. */
            500: {
                headers: {
                    [name: string]: unknown;
                };
