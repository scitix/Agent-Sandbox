...
function filterKey(f: SandboxFilters) {
  return [f.cluster, f.team ?? "", f.user ?? "", f.pool ?? "", f.sandboxEnv ?? ""]
}
...
// ─── Env detail: capacity waterfall (per-pool stacked replicas) ────────────

export const envCapacityWaterfallQueryOptions = (
  filters: SandboxFilters,
  preset: TimeRangePreset,
  options?: { refetchInterval?: number },
) =>
  queryOptions({
    queryKey: ["prometheus", "env-capacity-waterfall", ...filterKey(filters), preset],
    queryFn: () =>
...
export const envCapacityWaterfallAbsoluteQueryOptions = (
  filters: SandboxFilters,
  start: number,
  end: number,
  step: string,
...
// ─── Env detail: per-pool sparklines (compact desired+running per pool) ─────

export const envPoolSparklinesQueryOptions = (
  filters: SandboxFilters,
  preset: TimeRangePreset,
  options?: { refetchInterval?: number },
) =>
  queryOptions({
    queryKey: ["prometheus", "env-pool-sparklines", ...filterKey(filters), preset],
    queryFn: () =>
      prometheusGet<TimeSeriesData>("env-pool-sparklines", {
        cluster: filters.cluster,
        team: filters.team,
        user: filters.user,
        sandbox_env: filters.sandboxEnv,
