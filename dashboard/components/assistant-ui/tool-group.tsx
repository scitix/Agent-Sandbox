...
// `rounded-2xl`, same as the user bubble and the reasoning block: these are the
// framed blocks a message is made of, stacked in one column, and three different
// corner radii read as three unrelated widgets. The tool cards INSIDE this frame
// (`tool-fallback`) deliberately keep a tighter radius — an inner corner should
// be smaller than the one containing it.
const toolGroupVariants = cva('aui-tool-group-root group/tool-group w-full', {
  variants: {
    variant: {
      outline: 'rounded-2xl border py-3',
      ghost: '',
      muted: 'border-muted-foreground/30 bg-muted/30 rounded-2xl border py-3',
    },
  },
...
function ToolGroupTrigger({
  ...
  // Two explicit keys rather than an i18next plural suffix: TranslationKey is
  // generated from the dictionary, and `_one`/`_other` variants have no base
  // key to type against.
  const label =
    count === 1
      ? t('assistant.tool.callCountOne')
      : t('assistant.tool.callCountMany', { count })
  ...
  return (
    ...
    >
      ...
      {/* Base UI marks an open trigger with `data-panel-open` and never sets
          `data-state`, so the old pair of state selectors matched nothing and the
          chevron pointed down whether the group was open or shut. */}
      <ChevronDownIcon
