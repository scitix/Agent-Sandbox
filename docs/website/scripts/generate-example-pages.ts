...
const GROUPS: Array<{ dir: string; title: string; examples: Example[] }> = [
  ...
  },
  {
    ...
    examples: [
      {
        file: 'e2b_sandboxenv.yaml',
        slug: 'e2b',
        title: 'A warm pool',
        summary: 'An environment on the E2B template, with two Pods kept warm — and the one line to change for the other two templates.',
      },
    ],
  },
];
...
function page(example: Example): string {
  ...
  return [
    ...
    `[\`config/samples/${example.file}\`](${REPO}/${example.file}) on the \`develop\``,
