...
Secrets live in a **vault** that is write-only: you can list names and
overwrite them, never read them back. In the console it is the **Vault** page;
through the SDK it is the E2B `/secrets` surface, which needs the env's
endpoints:
...
Reference a secret **by name** on the create call. A plaintext credential in
the rules is refused with `400` — deliberately, because accepting it would put
the value in the request body, the access log and the caller's source, which is
the exposure the feature exists to remove.
