...
```
created ──▶ Idle (idle image; no workload)
               │  claim: image swapped in place, container restarts
               ▼
            Running (your image; your sandbox)
               │  sandbox killed / idle timeout: container restarts, metadata cleared
               ▼
            Idle again
```

1. **Idle.** The Pod runs the template's `idleImage` — the sandbox runtime
   sitting in front of no workload. This is what makes it cheap to keep Pods
   around: 20 idle Pods are not 20 copies of your image.
2. **Claim.** The platform picks an idle Pod and replaces the main container's
   image with the one the create asked for. Kubernetes restarts that container
   on the same Pod — no scheduling, no new Pod, no volume re-attach.
3. **Running.** Your process starts, and the sandbox is handed to the caller.
4. **Return.** When the sandbox is killed or times out, the container is
   restarted onto the idle image and the previous sandbox's metadata is
   cleared, so the next claim does not inherit it.
